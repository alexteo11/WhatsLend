  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        day: "h-9 w-9 text-center text-sm p-0 relative [&:has([aria-selected].selected)]:hidden [&:has([aria-selected].day-outside)]:bg-yellow-500/50 bg-purple [&:has([aria-selected])]:bg-blue-500 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 rounded-1",
        day_button: cn(
          buttonVariants({ variant: "ghost" }),
          "h-9 w-9 p-0 font-normal aria-selected:opacity-100 rounded-l-md rounded-r-md group-hover:bg-primary group-hover:text-primary-foreground",
        ),
        range_end: "day-range-end",
        selected:
          "group bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground focus:bg-primary focus:text-primary-foreground rounded-l-md rounded-r-md",
        today: "border border-app bg-accent text-accent-foreground",
        outside:
...
>(
  (
    ...
    const [confirmDate, setConfirmDate] = React.useState(value);
    ...
    const displayConfirmValue = (): React.ReactNode => {
      if (!confirmDate) {
      ...
      return <span>{format(confirmDate, "PPP", { locale: loc })}</span>;
    };
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild disabled={disabled}>
          <Button
            variant="outline"
            className={cn(
              "application__form-input w-full justify-between text-left font-normal",
              !displayDate && "text-muted-foreground",
              className,
              "[&:has(+p.text-destructive)]:border-none [&:has(+p.text-destructive)]:ring-1 [&:has(+p.text-destructive)]:ring-destructive",
            )}
            ref={buttonRef}
          >
            {displayConfirmValue()}
        ...
        <PopoverContent className="w-auto p-0">
          <Calendar
            ...
            yearRange={yearRange}
            locale={locale}
            disabled={props.calendarDisabledRange}
            {...props}
          />
          {granularity !== "day" && (
